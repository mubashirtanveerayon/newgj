...
func on_state_execution_finished():
	...
	#print(wave_index==waves.size()-1,wave_index,waves.size())
	$UI.update_wave_duration(current_state.time_in_minutes * 60,wave_index>=waves.size())
...
func _ready():
	...
	$UI.update_wave_duration(current_state.time_in_minutes * 60,false)
